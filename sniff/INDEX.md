...
| PATCH.h | 3-way worktree merge (shared verb `patch`, see `VERBS.md` §PATCH).  `PATCHApply(reporoot, target)` resolves ours (branch tip from `.sniff/at.log`), theirs (`target` via `REFSResolve` or 40-hex sha), and LCA (via `GRAFLca`), then walks the three trees by fetching `GRAFGet <dir>/?<sha>` at each level and applies per-leaf actions: take-theirs, take-ours (noop), 3-way merge via `GRAFGet <path>?<ours>&<theirs>`, structural add/delete against LCA, and modify/delete conflict surfacing.  Dirty wt files refuse up front (`PATCHDIRTY`).  Conflicts land as inline `<<<<<<<` markers; exit code signals `PATCHCONFLICT`.  Every byte on disk comes through graf — sniff owns only wt writes and registry updates. |
