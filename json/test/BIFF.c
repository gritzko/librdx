...
static ok64 BIFFCheckJSON(u8csc data, char const *expected) {
    ...
    u8cs got = {u8bDataHead(jbuf), u8bIdleHead(jbuf)};
...
#define BIFF_DATA(name) \
    ((u8c *[]){u8bDataHead(name##_buf), u8bIdleHead(name##_buf)})
...
ok64 BIFFtestMergeIdentityLeft() {
    ...
    u8cs ld = {u8bDataHead(left_buf), u8bIdleHead(left_buf)};
    u8cs rd = {u8bDataHead(right_buf), u8bIdleHead(right_buf)};
    call(BASONMerge, out_buf, out_idx, lstk, ld, rstk, rd);

    u8cs od = {u8bDataHead(out_buf), u8bIdleHead(out_buf)};
...
ok64 BIFFtestDiffIdentical() {
    ...
    u8cs od = {u8bDataHead(old_buf), u8bIdleHead(old_buf)};
    u8cs nd = {u8bDataHead(neu_buf), u8bIdleHead(neu_buf)};
...
ok64 BIFFtestDiffScalar() {
    ...
    u8cs dd = {u8bDataHead(out_buf), u8bIdleHead(out_buf)};
...
ok64 BIFFtestDiffRoundtrip() {
    ...
    u8cs od2 = {u8bDataHead(old_buf), u8bIdleHead(old_buf)};
    u8cs dd = {u8bDataHead(diff_buf), u8bIdleHead(diff_buf)};
    call(BASONMerge, merge_buf, merge_idx, lstk2, od2, rstk2, dd);

    u8cs md = {u8bDataHead(merge_buf), u8bIdleHead(merge_buf)};
...
static ok64 BIFFCheckJSONEqual(u8csc a_data, u8csc b_data) {
    ...
    u8cs aj = {u8bDataHead(abuf), u8bIdleHead(abuf)};
    u8cs bj = {u8bDataHead(bbuf), u8bIdleHead(bbuf)};
...
ok64 BIFFtestFuzzRepros() {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        u8cs dd = {u8bDataHead(dbuf), u8bIdleHead(dbuf)};
        ...
        u8cs md = {u8bDataHead(mbuf), u8bIdleHead(mbuf)};
...
ok64 BIFFtestFromRaw(u8csc raw) {
    ...
    u8cp od0 = u8bDataHead(old_buf), od1 = u8bIdleHead(old_buf);
    u8cs odata = {od0, od1};
    u8cp nd0 = u8bDataHead(neu_buf), nd1 = u8bIdleHead(neu_buf);
    u8cs ndata = {nd0, nd1};
    ...
    u8cp dd0 = u8bDataHead(dbuf), dd1 = u8bIdleHead(dbuf);
    u8cs dd = {dd0, dd1};
    ...
    // Dump diff JSON
    {
        ...
        u8cs dj = {u8bDataHead(djbuf), u8bIdleHead(djbuf)};
        fprintf(stderr, "  diff json: %.*s\n", (int)$len(dj), dj[0]);
    }
    call(BASONMerge, mbuf, NULL, ls, odata, rs, dd);
    u8cp md0 = u8bDataHead(mbuf), md1 = u8bIdleHead(mbuf);
    u8cs md = {md0, md1};
...
ok64 BIFFtestMergeN2way() {
    ...
    BIFFN_SETUP(2048, 4);
    u8cs ad = {u8bDataHead(a_buf), u8bIdleHead(a_buf)};
    u8cs bd = {u8bDataHead(b_buf), u8bIdleHead(b_buf)};
    _ninputs[0][0] = ad[0]; _ninputs[0][1] = ad[1];
...
ok64 BIFFtestMergeN3way() {
    ...
    u8cs ad = {u8bDataHead(a_buf), u8bIdleHead(a_buf)};
    u8cs bd = {u8bDataHead(b_buf), u8bIdleHead(b_buf)};
    u8cs cd = {u8bDataHead(c_buf), u8bIdleHead(c_buf)};
    _ninputs[0][0] = ad[0]; _ninputs[0][1] = ad[1];
    _ninputs[1][0] = bd[0]; _ninputs[1][1] = bd[1];
...
static ok64 BIFFDumpText(char const *label, u8csc data) {
    ...
    u8cs t = {u8bDataHead(tbuf), u8bIdleHead(tbuf)};
    fprintf(stderr, "--- %s ---\n%.*s\n", label, (int)$len(t), t[0]);
    done;
}
...
ok64 BIFFtestExportText() {
    ...
    u8cs got = {u8bDataHead(tbuf), u8bIdleHead(tbuf)};
