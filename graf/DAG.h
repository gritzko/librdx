...
//  Check if a commit (by hashlet) is a merge (has >1 parent).
fun b8 DAGIsMerge(dag_stack const *st, u64 commit_hashlet) {
    u64 key_lo = DAGPack(DAG_COMMIT_PARENT, 0, commit_hashlet);
    u64 key_hi = DAGPack(DAG_COMMIT_PARENT, DAG_GEN_MASK, commit_hashlet);
    u32 count = 0;
    for (u32 r = 0; r < st->n && count < 2; r++) {
        belt128cp base = st->runs[r][0];
        size_t len = (size_t)(st->runs[r][1] - base);
        size_t lo = 0, hi = len;
        while (lo < hi) {
        ...
        while (lo < len && base[lo].a >= key_lo && base[lo].a <= key_hi) {
            if (DAGType(base[lo].a) == DAG_COMMIT_PARENT) count++;
            lo++;
        }
    }
    return count > 1;
...
//  Find BLOB_COMMIT for a blob, preferring non-merge commits.
//  Falls back to a merge commit if no non-merge entry exists.
fun belt128cp DAGBlobCommit(dag_stack const *st, u64 blob_hashlet) {
    u64 key_lo = DAGPack(DAG_BLOB_COMMIT, 0, blob_hashlet);
    u64 key_hi = DAGPack(DAG_BLOB_COMMIT, DAG_GEN_MASK, blob_hashlet);
    belt128cp fallback = NULL;
    for (u32 r = 0; r < st->n; r++) {
        belt128cp base = st->runs[r][0];
        size_t len = (size_t)(st->runs[r][1] - base);
        size_t lo = 0, hi = len;
        ...
        while (lo < len && base[lo].a >= key_lo && base[lo].a <= key_hi) {
            if (DAGType(base[lo].a) == DAG_BLOB_COMMIT) {
                u64 ch = DAGHashlet(base[lo].b);
                if (!DAGIsMerge(st, ch)) return &base[lo];
                if (!fallback) fallback = &base[lo];
            }
            lo++;
        }
    }
...
//  Find PREV_BLOB for a blob, preferring entries from non-merge commits.
fun belt128cp DAGPrevBlob(dag_stack const *st, u64 blob_hashlet) {
    u64 key_lo = DAGPack(DAG_PREV_BLOB, 0, blob_hashlet);
    u64 key_hi = DAGPack(DAG_PREV_BLOB, DAG_GEN_MASK, blob_hashlet);
    belt128cp fallback = NULL;
    for (u32 r = 0; r < st->n; r++) {
        belt128cp base = st->runs[r][0];
        size_t len = (size_t)(st->runs[r][1] - base);
        ...
        while (lo < len && base[lo].a >= key_lo && base[lo].a <= key_hi) {
            if (DAGType(base[lo].a) == DAG_PREV_BLOB) {
                // Check if the commit that has this blob is a merge
                // by looking up BLOB_COMMIT for this blob's hashlet
                u64 bh = DAGHashlet(base[lo].a);
                belt128cp bc = DAGBlobCommit(st, bh);
                b8 from_merge = NO;
                if (bc) from_merge = DAGIsMerge(st, DAGHashlet(bc->b));
                if (!from_merge) return &base[lo];
                if (!fallback) fallback = &base[lo];
            }
            lo++;
        }
