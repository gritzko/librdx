...
fun ok64 X(SKIP, feed)(Bu8 buf, X(SKIP, tab) * k) {
    sane(Bok(buf) && k != nil);
...
fun ok64 X(SKIP, drain)(X(SKIP, tab) * hop, Bu8 buf, size_t pos) {
...
fun ok64 X(SKIP, load)(X(SKIP, tab) * k, Bu8 buf) {
    sane(k != nil && Bok(buf));
    zerop(k);
...
fun ok64 X(SKIP, find)(u8c$ range, Bu8 hay, $u8c needle, $cmpfn cmp) {
