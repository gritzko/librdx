...
int d2s_buffered_n(double f, char* result) {
    ...
    if (isSmallInt) {
        ...
        // trailing (decimal) zeros. For scientific notation we need to move
        // these zeros into the exponent. (This is not needed for fixed-point
        // notation, so it might be beneficial to trim trailing zeros in
        // to_chars only if needed - once fixed-point notation output is
        // implemented.)
        for (;;) {
